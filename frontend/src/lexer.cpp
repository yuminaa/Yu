 ...
 * Design Philosophy:
 * -----------------
 * This lexer is built on three core design principles:
 *
 * 1. Data-Oriented Design (DOD):
 *    - Cache-aligned lookup tables for character classification
 *    - SIMD-friendly data structures for bulk processing
 *    - Dense memory layout with minimal indirection
 *    - Zero-copy token generation using string_view
 *
 * 2. Data-Driven Architecture:
 *    - Behavior defined through lookup tables rather than branching logic
 *    - Token classification and validation rules as data
 *    - State management via bitfield flags
 *    - Extensible through data structure updates
 *
 * 3. Pipeline Architecture:
 *    - Single-threaded lexer as optimized token producer
 *    - Feeds into multi-threaded behavior tree parser
 *    - Zero-copy handoff between stages
 *    - Lock-free token stream access
 *
 * Performance Characteristics:
 * --------------------------
 * - Three-tier cache prefetching strategy (L1/L2/L3)
 * - Branchless operations using lookup tables
 * - SIMD-optimized whitespace and comment skipping
 * - Cache-conscious memory layout
 * - Single producer eliminates synchronization overhead
 *
 * Integration with Parser:
 * ----------------------
 * The lexer is specifically designed to work with a parallel behavior tree parser:
 * - Produces sequential, deterministic token stream
 * - Token data layout optimized for parallel consumption
 * - Error flags support non-blocking error handling
 * - Memory layout supports NUMA-aware processing
 *
 * Implementation Notes:
 * -------------------
 * - Uses compile-time initialization for lookup tables
 * - HOT_FUNCTION and ALWAYS_INLINE hints for critical paths
 * - LIKELY/UNLIKELY annotations for branch prediction
 * - Careful alignment and padding for optimal cache usage
 *
 * Extension Points:
 * ----------------
 * - Token types can be added via enum and lookup tables
 * - Character classification through char_type table
 * - Error handling through flag system
 * - Token validation rules in lookup tables
 *
 * @note This implementation prioritizes single-threaded performance as the parser
 *       handles parallelization through its behavior tree design. This division
 *       of responsibilities allows each component to be optimized for its specific role.
 */
